...
@push('meta-seo')
    <title>Data Pemeriksaan Pasien - Posbindu</title>
    <meta content="Halaman Data Pemeriksaan Pasien Posbindu" name="description">
                ...
                <div class="card-body pt-3 pb-md-1">
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-6 text-start text-md-end">
                            <a href="{{ route('be.pemeriksaan.list') }}" class="btn btn-dark fw-semibold">Kembali</a>
                        </div>
                    </div>
                </div>
            </div>
...
@section('content')
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <h6 class="my-3">Menampilkan <strong>{{ $pemeriksaanAmount }}</strong> data pemeriksaan.</h6>
                    <div class="row">
                        {{-- @for ($x = 1; $x <= 4; $x++) --}}
                        @forelse ($getPemeriksaan as $item)
                            <div class="col-lg-6">
                                <div class="card w-100 border">
                                    <div class="card-body">
                                        {{-- <p class="card-subtitle mb-4">Silakan isi data diri anda dengan benar.</p> --}}
                                        <dl class="row text-dark">
                                            <dt class="col-sm-7">Tanggal Pemeriksaan</dt>
                                            <dd class="col-sm-5 fw-bolder">
                                                {{ Carbon\Carbon::parse($item->tgl_input)->translatedFormat('d F Y') }}
                                            </dd>

                                            <h6 class="fw-semibold text-muted mt-2 fs-4">Keluhan Utama</h6>

                                            <dd class="col-sm-12 text-capitalize">
                                                {{ $item->keluhan_utama }}
                                            </dd>

                                            <h6 class="fw-semibold text-muted mt-2 fs-4">Pemeriksaan Fisik</h6>
                                            ...
                                            <dt class="col-sm-7">Sistol</dt>
                                            <dd class="col-sm-5 text-capitalize">
                                                {{ $item->sistol }} mmHG
                                            </dd>

                                            <dt class="col-sm-7">Diastol</dt>
                                            <dd class="col-sm-5 text-capitalize">
                                                {{ $item->diastol }} mmHG
                                            </dd>
                                        ...
                                        <div class="d-flex">
                                            ...
                                            <button class="btn btn-sm btn-danger fw-semibold d-flex align-items-center py-2"
                                                data-bs-toggle="modal" data-bs-target="#delPemeriksaan"
                        ...
                        @empty
                            <h6 class="fw-semibold text-center">Belum ada Pemeriksaan.</h6>
                        @endforelse
                        {{-- @endfor --}}
                    </div>
                </div>
    ...
    {{-- Modal Delete --}}
    <div class="modal fade" tabindex="-1" id="delPemeriksaan" tabindex="-1" aria-labelledby="delPemeriksaanLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content modal-filled bg-dark">
                <form action="{{ route('be.pemeriksaan.act_delete') }}" method="post">
                    @csrf
                    <div class="modal-body p-4">
                        <div class="text-center">
                            ...
                            <input type="hidden" id="del-pemeriksaan_id" name="pemeriksaan_id" value="">
...
@push('js')
    <script>
        function edtData(id) {
            window.location.replace("{{ route('be.pemeriksaan.edit') }}" + "/" + btoa(id));
        }

        function delData(id) {
            $('#del-pemeriksaan_id').val(id);
        }
    </script>
@endpush
